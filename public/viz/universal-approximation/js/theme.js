/* Theme toggle. Light is the lecture default, because a lit hall washes a dark
 * page out. Dark is for preparing the lecture at night. The choice persists in
 * localStorage, follows the operating system on a first visit, and answers the
 * 't' key so the lecturer can flip it mid sentence.
 *
 * Every colour lives in a CSS custom property, so a switch is one attribute on
 * <html> and every chart follows by itself. A scene that caches a resolved
 * colour would break that, which is why no scene may resolve one: charts use
 * the classes in css/style.css. Scenes still listen for 'theme-change' through
 * Plot.mount, because a redraw is needed when the geometry depends on text
 * metrics.
 *
 * Globals used: none. */

window.Theme = (function () {
  const root = document.documentElement;
  const KEY = 'ua-viz-theme';

  function get() { return root.getAttribute('data-theme') || 'light'; }

  function set(t) {
    root.setAttribute('data-theme', t);
    try { localStorage.setItem(KEY, t); } catch (e) { /* private browsing */ }
    window.dispatchEvent(new CustomEvent('theme-change', { detail: { theme: t } }));
  }

  function toggle() { set(get() === 'dark' ? 'light' : 'dark'); }

  function init() {
    let saved = null;
    try { saved = localStorage.getItem(KEY); } catch (e) { /* private browsing */ }

    // A forced theme in the URL wins, so a headless capture can ask for either
    // one without touching storage. Dev affordance, not a user feature.
    const forced = (window.location.hash || '').match(/theme=(light|dark)/);
    if (forced) saved = forced[1];

    if (!saved) {
      saved = (window.matchMedia && matchMedia('(prefers-color-scheme: dark)').matches)
        ? 'dark' : 'light';
    }
    set(saved);

    window.addEventListener('keydown', e => {
      if (e.target && /input|textarea|select/i.test(e.target.tagName || '')) return;
      if (e.key === 't' || e.key === 'T') toggle();
    });
  }

  return { get, set, toggle, init };
})();
