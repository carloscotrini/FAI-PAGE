/* Scene 10. Takeaways.
 *
 * The close. Seven lines, one per chapter beat, each one anchored to a number
...
window.scenes.scene10 = function (root) {
  ...
  // 4. The Scrabble model, read at the first stop where nonsense fits the
  // training countries perfectly: where scene 6 opens.
  const S = D.scrabble;
  let sAt = S.trainR2.findIndex(v => v >= 0.9995);
  if (sAt < 0) sAt = S.ps.length - 1;

  // 5. The patch. Lasso at its best alpha.
  const lasso = D.patch.models.lasso;

  // 6. The reveal. Two models that predict alike and explain differently.
  const R = D.reveal;

  // 7. The trust. OLS, one split against a hundred.
  ...
  root.appendChild(UI.head(
    "The close",
    "Takeaways.",
    "Seven lines. Every number on them appeared earlier in this viz."));

  const layout = UI.el("div.scene-layout.s10-grid");
  const list = UI.el("div.s10-list");
  const rail = UI.el("div.s10-rail");
  ...
  function fig(v) { return UI.el("span.s10-fig", String(v)); }

  function item(n, lead, parts) {
    const body = UI.el("div.s10-body", UI.el("span.s10-lead", lead), " ");
    parts.forEach(p => {
      body.appendChild(p instanceof Node ? p : document.createTextNode(String(p)));
    });
    return UI.el("div.s10-item", UI.el("div.s10-num", String(n)), body);
  }
  ...
  list.appendChild(item(4, "Even nonsense fits perfectly.", [
    "With ", fig(S.ps[sAt]), " nonsense columns the training score is ",
    fig(Stats.r2(S.trainR2[sAt])), " and the test score is ",
    fig(Stats.r2(S.testR2[sAt])), ".",
  ]));

  list.appendChild(item(5, "Regularisation rescues the prediction.", [
  ...
  list.appendChild(item(6, "A rescued prediction buys you no explanation.", [
  ...
  list.appendChild(item(7, "One split is not evidence.", [
    "Seed ", fig(D.trust.singleSeed), " handed OLS ",
    fig((olsTrust.single >= 0 ? "+" : "") + Stats.r2(olsTrust.single)), ". Its median over ",
  ...
  const sparkHost = UI.el("div.viz-wrap.s10-spark");
  ...
  Plot.mount(sparkHost, function (g, iw, ih) {
    ...
    g.append("text").attr("class", "s10-mark-label")
  ...
  rail.appendChild(UI.el("p.s10-cap",
